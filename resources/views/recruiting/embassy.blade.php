...
@section('content')
                ...
                <div class="py-6 flex md:items-center md:justify-between lg:border-t lg:border-gray-200">
                    <div class="flex-1 min-w-0">
                        <div class="flex items-center">
                            <div>
                                <div class="flex items-center">
                                    <h1 class="text-2xl font-bold leading-7 text-gray-900 sm:leading-9 sm:truncate">Embassy
                                    </h1>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex justify-end">
                            <div class="relative lg:mx-0">
                                <form action="{{ route('passport.search') }}" method="get">
                                    ...
                                    <input type="hidden" name="search_id" value="embassy">
                                    <input name="q"
                                            ...
                                            <label for="passportStatus{{$key}}"
                                                ...
                                                <div class="relative">
                                                    <input id="passportStatus{{$key}}"
                                                           ...
                                                           type="checkbox"
                                                           class="sr-only"
                                                           {{ $passport->embassy ? 'checked' : '' }} data-id="{{$passport->id}}"/>
                                                    <div
                                                        class="{{ $passport->embassy ? 'bg-blue-600' : 'bg-gray-200' }} relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                                                        role="switch" aria-checked="false"
                                                        id="toggle_back_{{$passport->id}}">
                                                        <span class="sr-only">Use setting</span>
                                                        <!-- Enabled: "translate-x-5", Not Enabled: "translate-x-0" -->
                                                        <span aria-hidden="true"
                                                              class="{{ $passport->embassy ? 'translate-x-5' : 'translate-x-0' }} pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow transform ring-0 transition ease-in-out duration-200"
    ...
    <script>
        function ttcStatusChange(source) {
            ...
            let url = "{{ route('passport.embassy.status')}}";
